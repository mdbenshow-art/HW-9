...
# Load and prepare movie context
if os.path.exists(MOVIES_FILE):
    try:
        with open(MOVIES_FILE, 'r', encoding='utf-8') as f:
            movies_data = json.load(f)
            
        # Optimize context structure to reduce token usage
        optimized_list = []
    ...
    print("警告: 找不到 movies.json，Chatbot 將以無數據狀態運行。")
...
def chat():
    try:
        ...
        # Dynamically detect available models in the new SDK to avoid 404 deprecation errors
        ...
        try:
            ...
            print(f"[{time.strftime('%X')}] 使用全新 google-genai 套件，動態解析獲得適用之模型: {selected_model}")
        ...
        # Call generate_content in the new SDK
...
if __name__ == '__main__':
    print("CineScrape 伺服器啟動於 http://localhost:8000")
